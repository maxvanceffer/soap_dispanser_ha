...
void ClockScreen::draw(ScreenManager* manager) {
  ...
  IService* timeService = Container::getInstance()->getService("Time");
  if (!timeService) {
    manager->setFont(u8g2_font_6x12_tf);
    manager->drawStr(10, 30, "Time service not available");
    return;
  }
  // Get current time using time() function instead of trying to access TimeManager directly
